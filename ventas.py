...
class Ventas(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        self.numero_factura_actual = self.obtener_numero_factura_actual()  # Inicializar el número de factura desde la base de datos
        self.widgets()
        self.mostrar_numero_factura()  # Esta llamada debe ser después de que los widgets estén inicializados
    ...
    def widgets(self):
        ...
        titulo = tk.Label(frame1, text="VENTA DE PRODUCTOS", font="sans 30 bold", bg="#dddddd", anchor="center")
        titulo.pack()
        ...
        # Botones
        boton_agregar = tk.Button(lblframe1, text="Agregar Artículo", bg="#3498db", fg="white", font="sans 16 bold", command=self.registrar)
        boton_agregar.place(x=50, y=10, width=240, height=50)

        boton_pagar = tk.Button(lblframe1, text="Pagar", bg="#2ecc71", fg="white", font="sans 16 bold", command=self.abrir_ventana_pago)
        boton_pagar.place(x=400, y=10, width=240, height=50)

        # Botón para abrir la ventana de facturas
        boton_ver_facturas = tk.Button(lblframe1, text="Ver Facturas", bg="#34495e", fg="white", font="sans 16 bold", command=self.abrir_ventana_facturas)
        ...
        copyright_label = tk.Label(frame2, text="© 2024 InnovaSoft Code. Todos los derechos reservados.", font="sans 12 bold", bg="#C6D9E3", fg="gray")
        copyright_label.place(x=320, y=500)
    ...
    def actualizar_precio(self, event):
        ...
        try:
            ...
            precio = c.fetchone()
            if (precio):
                self.entry_valor.config(state='normal')  # Permitir edición para actualizar
                self.entry_valor.delete(0, tk.END)
                self.entry_valor.insert(0, precio[0])
                self.entry_valor.config(state='readonly')  # Volver a establecer como solo lectura
            else:
                self.entry_valor.config(state='normal')  # Permitir edición para actualizar
                self.entry_valor.delete(0, tk.END)
    ...
    def mostrar_numero_factura(self):
        self.numero_factura.set(self.numero_factura_actual)  # Actualizar el valor del StringVar
    ...
    def abrir_ventana_pago(self):
        ...
        label_total = tk.Label(ventana_pago, bg="#C6D9E3", text=f"Total a pagar: COP {self.obtener_total():.0f}", font="sans 18 bold")
        label_total.place(x=70, y=20)

        # Entry para cantidad pagada
        ...
        def calcular_cambio():
            try:
                ...
                if cambio < 0:
                    ...
                    return
                label_cambio.config(text=f"Vuelto: COP {cambio:.0f}")
            except ValueError:
                messagebox.showerror("Error", "Cantidad pagada no válida")

        # Botón para calcular el cambio
        boton_calcular = tk.Button(ventana_pago, text="Calcular Vuelto", bg="#3498db", fg="white", font="sans 12 bold", command=calcular_cambio)
        boton_calcular.place(x=100, y=240, width=240, height=40)

        # Botón para pagar
        boton_pagar = tk.Button(ventana_pago, text="Pagar", bg="#2ecc71", fg="white", font="sans 12 bold", command=lambda: self.pagar(ventana_pago, entry_cantidad_pagada, label_cambio))
        boton_pagar.place(x=100, y=300, width=240, height=40)

    def abrir_ventana_facturas(self):
        # Crear una nueva ventana Toplevel para mostrar las facturas
        ...
        # TreeView para mostrar la lista de facturas
        ...
        try:
            ...
            try:
                for child in self.tree.get_children():
                    ...
                    # Verificar nuevamente el stock antes de registrar la venta
